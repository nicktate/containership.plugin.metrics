                ...
                (req, res) => {
                    return api.getApplications((err, apps) => {
                        ...
                        return api.getHosts((err, hosts) => {
                            ...
                            const targetContainer = _.first(containers);

                            const targetHost = _.flow(
                                _.partial(_.values, _),
                                _.partial(_.filter, _, (host) => {
                                    return _.some(host.containers, (c) => {
                                        return (c.name || c.application) === 'containership-prometheus' &&
                                            (c.container_id || c.id) === targetContainer.id;
                                    });
                                }),
                                _.partial(_.first, _))(hosts);

                            const container_ip = targetContainer.internal_container_ip || _.get(targetHost, ['address', 'private']);
                            const container_port = targetContainer.container_port || targetContainer.host_port || 9090;
