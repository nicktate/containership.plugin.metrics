        ...
        prometheus_metrics: function(req, res) {
            ...
            return core.applications.get_containers('containership-prometheus', (err, containers) => {
                ...
                return core.cluster.myriad.persistence.get([core.constants.myriad.APPLICATION_PREFIX, 'containership-prometheus'].join(core.constants.myriad.DELIMITER), (err, app) => {
                    ...
                    let PROMETHEUS_PORT = prometheusContainer.env_vars.PROMETHEUS_PORT || 9090;

                    // hacky... new version of prometheus server runs in bridge mdoe and has been moved
                    // to a new dockerhub repo called prometheus-metric-server. if that is the image running,
                    // use the discovery port, if not expect it to be running in host mode
                    if (app.image.indexOf('prometheus-metric-server') >= 0) {
                        PROMETHEUS_PORT = app.discovery_port;
                    }
